...
from constants.tests import SAVED_ALBUM_TYPE, SAVED_ALBUM_FILE_NAME
from datetime import datetime, timezone


@pytest.mark.parametrize("data", [SAVED_ALBUM_FILE_NAME], indirect=True)
def test_tracks_model(data, model_factory):
    ...
    optional = []
    model_factory(SAVED_ALBUM_TYPE, data, required, optional)
    

    
@pytest.mark.parametrize("data", [SAVED_ALBUM_FILE_NAME], indirect=True)
def test_saved_album_added_at_valid(data):
    """
    Vérifie que 'added_at' accepte un timestamp ISO timezone-aware dans le passé.
    """
    model_cls = SAVED_ALBUM_TYPE
    d = data.copy()
...
@pytest.mark.parametrize("data", [SAVED_ALBUM_FILE_NAME], indirect=True)
def test_saved_album_added_at_naive_error(data):
    """
...
@pytest.mark.parametrize("data", [SAVED_ALBUM_FILE_NAME], indirect=True)
def test_saved_album_added_at_future_error(data):
    """
